...
export class ProjectNavigationComponent implements OnChanges, OnInit {
  ...
  public dailyScrumVisible = false;
  ...
  public ngOnInit() {
    ...
    if (this.section === 'dailyscrum' || this.section === 'issues') {
      this.dailyScrumVisible = true;
    }
  }
  ...
  public popupDailyScrum(event: MouseEvent) {
    if (!this.collapsed) {
      return;
    }

    const children: ProjectMenuDialog['children'] = [
      {
        text: this.translateService.instant('PROJECT.SECTION.DAILY_SCRUM_MEETING'),
        link: [this.baseHref, 'project', this.project.slug, 'dailyscrum'],
      },
      {
        text: this.translateService.instant('PROJECT.SECTION.IMPEDIMENTS'),
        link: [this.baseHref, 'project', this.project.slug, 'issues'], // Cambiar 'impediments' a 'issues'
      }
    ];

    this.initDialog(event.target as HTMLElement, 'dailyscrum', children);
  }
  ...
  public toggleDailyScrum() {
    if (this.collapsed) {
      this.router.navigate(['/project', this.project.slug, 'dailyscrum']);
    } else {
      this.dailyScrumVisible = !this.dailyScrumVisible;
      localStorage.setItem('projectnav-dailyscrum', String(this.dailyScrumVisible));
    }
  }
