...
class TaskboardController extends mixOf(taiga.Controller, taiga.PageMixin, taiga.FiltersMixin)
    ...
    ## Template actions
    addNewTask: (type, us) ->
        # Obtener el ID correctamente de la estructura de Taiga
        usId = us?._attrs?.id or us?.id or null
        
        console.log "addNewTask - usId extraído:", usId  # Debug
            ...
            when "standard" then @rootscope.$broadcast("genericform:new",
                {
                    ...
                    'project': @scope.project,
                    'sprintId': @scope.sprintId,
                    'usId': usId  # Usar el ID correcto
                })
            when "bulk" then @rootscope.$broadcast("taskform:bulk", @scope.sprintId, usId)
