...
$confirm, $q, attachmentsService, $template, $compile) ->
    link = ($scope, $el, attrs) ->
        ...
        schemas = {
            ...
            task: {
                ...
                data: (project) ->
                    return {
                        ...
                        project: project  # <-- AGREGAR ESTA LÍNEA
                    }
                initialData: (data) ->
                    console.log "Task initialData - data:", data, "usId:", data.usId  # <-- Agregar este log
                    return {
                        project: data.project.id
                        subject: ""
                        description: ""
            ...
            planning:
                objName: 'Planning Session'
                model: 'planning-sessions'
                template: "planning"
                params: {}

                data: (project) ->
                    console.log("🔄 Cargando datos para planning session, proyecto:", project.id)
                    
                    # Obtener el injector de Angular si no está disponible
                    if not angular.element(document.body).injector
                        console.error("❌ Injector no disponible")
                        return {
                            translationID: 'PLANNING_SESSION'
                            translationIDPlural: 'PLANNING_SESSIONS'
                            availableUserStories: []
                            project: project
                        }
                    
                    injector = angular.element(document.body).injector()
                    $tgResources = injector.get("$tgResources")
                    $timeout = injector.get("$timeout")
                    $q = injector.get("$q")

                    # Usar el servicio de userstories que ya existe en Taiga
                    if $tgResources.userstories?.listAll
                        console.log("📌 Usando servicio userstories.listAll")
                        
                        return $tgResources.userstories.listAll(project.id, {milestone__isnull: true}).then (stories) ->
                            console.log("✅ Historias cargadas:", stories.length)
                            
                            # Filtrar solo las del backlog (sin milestone y no archivadas)
                            backlogStories = _.filter stories, (story) ->
                                return !story.milestone and !story.is_archived
                            
                            console.log("📚 Historias en backlog:", backlogStories.length)
                            
                            # Formatear historias para el lightbox
                            formattedStories = _.map backlogStories, (story) ->
                                return {
                                    id: story.id
                                    ref: story.ref
                                    title: story.subject
                            ...
                            return {
                                translationID: 'PLANNING_SESSION'
                                translationIDPlural: 'PLANNING_SESSIONS'
                                availableUserStories: formattedStories
                                project: project
                            }
                        .catch (error) ->
                        ...
                        console.error("❌ Servicio userstories no disponible")
                        return {
                            translationID: 'PLANNING_SESSION'
                            translationIDPlural: 'PLANNING_SESSIONS'
                            availableUserStories: []
                ...
                initialData: (data) ->
                    console.log("📥 Recibido en initialData:", data)
                    console.log("📋 Historias disponibles:", data?.availableUserStories?.length or 0)

                    selected = _.filter(data?.availableUserStories or [], (s) -> s.selected)
                    return {
                        project: data.project.id
                        name: ""
                        subject: ""
                        description: ""
                        tags: []
                        user_stories: _.map(selected, (s) -> s.id)
                        availableUserStories: data?.availableUserStories or []
                    }
        }
        ...
        $scope.$on "genericform:new", (ctx, params) ->
            console.log "genericform:new params recibidos:", params
            getSchema(params)
            $scope.mode = 'new'
        ...
        mount = (params) ->
            ...
            if $scope.mode == 'edit'
                ...
                continuarConMontaje(params)
            else
                maybePromise = schema.data($scope.project)
                if typeof maybePromise.then == "function"
                    maybePromise.then (responseData) ->
                        # Combinar responseData con params para no perder usId, sprintId, etc.
                        mergedData = _.extend({}, responseData, params)
                        console.log "Datos combinados:", mergedData  # Debug
                        continuarConMontaje(mergedData)
                else
                    # También aquí
                    mergedData = _.extend({}, maybePromise, params)
                    console.log "Datos combinados:", mergedData  # Debug
                    continuarConMontaje(mergedData)

            return

        continuarConMontaje = (data) ->
            console.log("🚀 continuarConMontaje iniciado")
            ...
            # Verificar que las historias existan y tengan la estructura correcta
            if $scope.obj.availableUserStories
                console.log("✅ Historias disponibles:", $scope.obj.availableUserStories.length)
                console.log("📋 Primera historia:", $scope.obj.availableUserStories[0]) if $scope.obj.availableUserStories.length > 0
                
                # Inicializar la propiedad selected en cada historia
                for story in $scope.obj.availableUserStories
                    story.selected = false unless story.hasOwnProperty('selected')
            
            # Inicializar el checkbox maestro EN EL SCOPE, no en obj
            $scope.selectAllStories = false

            # Primero definir las funciones helper
            
            # Función helper para obtener las historias seleccionadas
            $scope.getSelectedStories = ->
                return [] unless $scope.obj.availableUserStories
                return $scope.obj.availableUserStories.filter (story) -> story.selected
            
            # Actualizar user_stories cuando cambie la selección
            $scope.updateSelectedStories = ->
                selectedStories = $scope.getSelectedStories()
                $scope.obj.user_stories = selectedStories.map (story) -> story.id
                console.log("📝 Historias seleccionadas:", $scope.obj.user_stories.length)
                return

            # Función para seleccionar/deseleccionar todas
            $scope.toggleSelectAllStories = ->
                # Invertir manualmente el estado
                $scope.selectAllStories = !$scope.selectAllStories
                
                console.log("🔄 Toggle all stories, nuevo estado:", $scope.selectAllStories)
                return unless $scope.obj.availableUserStories?.length
                
                # Aplicar a todas las historias
                for story in $scope.obj.availableUserStories
                    story.selected = $scope.selectAllStories
                    
                # Actualizar la lista de historias seleccionadas
                $scope.updateSelectedStories()
                
                # Log para verificar
                selectedCount = $scope.getSelectedStories().length
                console.log("✅ Historias actualizadas. Seleccionadas:", selectedCount)
            ...
            # Función para verificar si todas están seleccionadas
            $scope.checkAllSelected = ->
                return false unless $scope.obj.availableUserStories?.length
                ...
                $scope.selectAllStories = allSelected
                # También actualizar las historias seleccionadas
                $scope.updateSelectedStories()
                return
        ...
        submit = debounce 2000, (event) ->
            ...
            # ✅ Asegurar historias seleccionadas
            if $scope.obj?.availableUserStories
                $scope.obj.user_stories = _
                    .chain($scope.obj.availableUserStories)
                    .filter((s) -> s.selected)
                    .map((s) -> s.id)
                    .value()  
            ...
            if $scope.mode == 'new'
                ...
                console.log("🔍 Verificando antes de crear:")
                console.log("- Modelo:", schema.model)
                console.log("- Objeto completo:", $scope.obj)
                console.log("- user_story ID:", $scope.obj.user_story)  # <-- Agregar este log específico
                console.log("- Historias seleccionadas:", $scope.obj.user_stories)
            ...
            promise.then (data) ->
                deleteAttachments(data).then () ->
                    createAttachments(data).then () ->
                        ...
                        # Broadcast específico para planning
                        if $scope.objType == 'planning'
                            $scope.$watch 'obj.subject', (newVal) ->
                                if newVal
                                    $scope.obj.name = newVal
                                    console.log("📝 Nombre actualizado desde subject:", newVal)
                            console.log("📢 Broadcasting planningform:new:success con data completa:", data)
                            # Asegurarse de enviar el objeto completo
                            $rootScope.$broadcast("planningform:new:success", data)
                            return
                        
                        # Broadcast genérico para otros tipos
                        if data.ref
